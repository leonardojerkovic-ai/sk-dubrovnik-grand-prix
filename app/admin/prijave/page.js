...
export default function AdminRegistrationsPage() {
  ...
  async function updateStatus(id, nextStatus) {
    ...
    const { error: rpcError } = await supabase.rpc('admin_update_registration_status', { p_registration_id: id, p_status: nextStatus });
    if (rpcError) setError(rpcError.message || 'Promjena statusa nije uspjela.');
    else { setMessage(`Status prijave je promijenjen u „${statusLabels[nextStatus]}”.`); await load(); }
    setBusyId(null);
  }

  if (loading) return <main className="admin-shell"><div className="admin-loading">Učitavanje prijava…</div></main>;
  if (!session) return <main className="admin-shell"><section className="admin-gate"><span className="section-kicker">Administracija</span><h1>Prijave na turnire</h1><p>Za pristup ovoj stranici potrebno je prijaviti se administratorskim računom.</p><a className="btn-primary" href="/admin/login">Prijava</a></section></main>;
  if (!isAdmin) return <main className="admin-shell"><section className="admin-gate"><span className="section-kicker">403</span><h1>Pristup nije dopušten</h1><p>Ova stranica je dostupna samo administratorima ŠK Dubrovnik Grand Prix.</p><a className="btn-secondary" href="/">Povratak na naslovnicu</a></section></main>;
