...
-- Keep registration statuses explicit and consistent.
...
-- A player can have at most one active registration for a tournament.
DROP INDEX IF EXISTS public.tournament_registrations_tournament_player_active_idx;
CREATE UNIQUE INDEX tournament_registrations_tournament_player_active_idx
  ON public.tournament_registrations (tournament_id, player_id)
  WHERE player_id IS NOT NULL AND status IN ('REGISTERED', 'CONFIRMED');

-- Rating eligibility must use the rating effective on the tournament date,
-- not a result row that does not exist yet at registration time.
...
BEGIN
  ...
  -- Restricted tournaments require valid DGP membership on the tournament date.
  ...
  IF v_t.tournament_scope = 'CATEGORY' AND v_t.category_code IS NOT NULL THEN
    ...
    IF v_rule.rule_type = 'AGE_MAX' THEN
      IF v_p.birth_year IS NULL THEN
        v_cat := false;
      ELSE
        v_age := extract(year FROM v_t.start_date)::integer - v_p.birth_year;
        v_cat := v_age <= v_rule.parameter_value;
      END IF;
    ELSIF v_rule.rule_type = 'AGE_MIN' THEN
      ...
      ELSE
        ...
        v_cat := v_age >= v_rule.parameter_value;
      END IF;
    ELSIF v_rule.rule_type = 'RATING_MAX' THEN
    ...
    ELSIF v_rule.rule_type = 'RATING_MIN' THEN
      SELECT COALESCE(pr.fide_standard, pr.fide_rapid, pr.fide_blitz)
        INTO v_rating
      FROM public.player_ratings pr
      WHERE pr.player_id = p_player_id
...
-- Public registration is performed through this function so that category,
-- deadline, publication, duplicate and capacity checks cannot be bypassed by
...
-- Public table INSERT is no longer the registration API. Admins keep full table access.
